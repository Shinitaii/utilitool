<script lang="ts">
	...
	const filteredProperties = $derived(
		searchQuery
			? properties.data.filter((p) => p.room_name.toLowerCase().includes(searchQuery.toLowerCase()))
			: properties.data
	);

	function findMeterGroupById(meterGroupId: string | undefined) {
		return (
			electricityMeters.find((m) => m.id === meterGroupId) ||
			waterMeters.find((m) => m.id === meterGroupId)
		);
	}
	...
	const filteredReadings = $derived.by(() => {
		...
		return readings.data.filter((r) => {
			const meterGroup = findMeterGroupById(r.meter_group_id);
	...
	const filteredBillings = $derived.by(() => {
		...
		return billings.data.filter((b) => {
			...
			const meterGroup = findMeterGroupById(prevReading.meter_group_id);
			return meterGroup?.utility_type === billingsUtilityFilter;
		});
	});

	function getMeterGroupName(meterGroupId: string): string {
		const found = findMeterGroupById(meterGroupId);
	...
	async function handleCreateProperty() {
		...
		try {
			...
			if (
				...
				newPropertyForm.meter_groups.electricity.trim()
			) {
	...
	async function handleUpdateProperty() {
		...
		try {
			...
			if (
				...
				editPropertyForm.meter_groups.electricity.trim()
			) {
				meter_groups.electricity = {
...
<div class="flex h-screen flex-col">
	<div class="space-y-4 border-b border-gray-200 bg-white p-6">
		<div class="flex items-center justify-between">
			...
			<a
				href={resolve('/properties/archive')}
		...
		<div class="flex w-64 flex-col border-r border-gray-200 bg-gray-50">
			<div class="space-y-3 border-b border-gray-200 p-4">
				...
				<button
					...
					class="flex w-full items-center justify-center rounded bg-blue-600 p-2 text-white hover:bg-blue-700"
					disabled={isLoading}
										...
										<table class="w-full text-sm">
											...
											<tbody>
												{#each filteredReadings as reading (reading.id)}
													{@const meterGroup = findMeterGroupById(reading.meter_group_id)}
													<tr class="border-b border-gray-200 hover:bg-gray-50">
														...
														<td class="px-6 py-4 font-mono text-gray-700">
															{formatReading(
																...
																meterGroup?.utility_type || 'electricity'
															)}
														</td>
														<td class="px-6 py-4 text-gray-600">
							...
							<div role="tabpanel" id="tab-panel-billings" aria-labelledby="tab-billings">
								...
								{#if filteredBillings.length === 0}
									<div class="p-6">
										<EmptyState
											title="No billings"
											message="Create a billing cycle to generate bills"
										/>
									</div>
								{:else}
									<div class="overflow-x-auto">
										<table class="w-full text-sm">
											...
											<tbody>
												{#each filteredBillings as billing (billing.id)}
													{@const prevReading = readings.data.find(
														(r) => r.id === billing.previous_reading_id
													)}
													{@const currReading = readings.data.find(
														(r) => r.id === billing.current_reading_id
													)}
													{@const meterGroup = prevReading
														? findMeterGroupById(prevReading.meter_group_id)
														: null}
													<tr class="border-b border-gray-200 hover:bg-gray-50">
														<td class="px-6 py-4 text-gray-700">
															{meterGroup?.meter_name || 'Unknown'}
							...
							<div role="tabpanel" id="tab-panel-history" aria-labelledby="tab-history">
								{#if submeterEntries.length === 0}
									<div class="p-6">
										<EmptyState
											title="No submeters"
											message="This property has no submeter-tracked meter groups"
										/>
									</div>
								{:else}
									<div class="space-y-6 p-6">
										{#each submeterEntries as [_utilityType, entry] (entry.meter_group_id)}
											{@const meterGroup = findMeterGroupById(entry.meter_group_id)}
											...
											<div class="rounded border border-gray-200">
												...
												>
													<div class="flex items-center gap-2">
														<span class="font-medium text-gray-800"
															>{getMeterGroupName(entry.meter_group_id)}</span
														>
														{#if meterGroup}
															<span
													...
													<button
														onclick={() =>
															handleRecordSubmeterReset(
																entry.meter_group_id,
																entry.current_version ?? 1
															)}
														disabled={resettingMeterGroupId === entry.meter_group_id}
														class="flex items-center gap-1.5 rounded px-3 py-1.5 text-sm font-medium text-orange-700 hover:bg-orange-100 disabled:opacity-50"
												...
												{#if versions.length === 0}
													<div class="p-4">
														<EmptyState
															title="No reset history"
															message="No resets have been recorded for this submeter"
														/>
													</div>
												{:else}
													<div class="overflow-x-auto">
																...
																{#each versions as [version, versionEntry] (version)}
																	<tr class="border-b border-gray-200 hover:bg-gray-50">
																		...
																		<td class="px-4 py-2 text-gray-600"
																			>{formatDateTime(toDate(versionEntry.reset_at))}</td
																		>
																		<td class="px-4 py-2 font-mono text-gray-700">
																			{meterGroup
																				? formatReading(
																						versionEntry.last_reading,
																						meterGroup.utility_type
																					)
																				: versionEntry.last_reading}
																		</td>
...
>
	<div class="space-y-4">
		...
		<div>
			<label for="edit-tenant-amount" class="block text-sm font-medium text-gray-700"
				>Tenant Amount</label
			>
			<input
