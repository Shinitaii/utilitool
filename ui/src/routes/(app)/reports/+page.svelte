...
<div class="space-y-6">
	...
	<div class="rounded-lg border border-gray-200 bg-white p-4">
		<div class="mb-4 grid grid-cols-2 gap-4">
			...
			<div>
				<div class="mb-1 block text-sm font-medium text-gray-700">End Date</div>
				<div class="flex items-center gap-2">
					...
					<label class="flex items-center gap-2 text-sm">
						<input type="checkbox" bind:checked={usePresent} class="rounded border-gray-300" />
	...
	{#if isLoading}
		...
		<div class="grid grid-cols-4 gap-4">
			{#each [1, 2, 3, 4] as _, i (i)}
				<div class="animate-pulse rounded-lg border border-gray-200 bg-white p-6">
					...
					<div class="mb-2 h-8 w-32 rounded bg-gray-200"></div>
		...
		<div class="grid grid-cols-2 gap-4">
			<div class="h-96 animate-pulse rounded-lg border border-gray-200 bg-white p-6"></div>
			<div class="h-96 animate-pulse rounded-lg border border-gray-200 bg-white p-6"></div>
		</div>
	{:else if summary}
		<!-- Summary Cards -->
		<div class="grid grid-cols-4 gap-4">
			...
			<div class="rounded-lg border border-gray-200 bg-white p-6">
				...
				<p class="mt-1 text-xs text-gray-500">
					{summary.paid_count} of {summary.paid_count +
						summary.pending_count +
						summary.overdue_count} paid
				</p>
			</div>
			<div class="rounded-lg border border-gray-200 bg-white p-6">
			...
			<div
				...
				style="background-color: var(--color-status-overdue-bg, #fee2e2)"
			>
			...
			<div class="grid grid-cols-3 gap-4">
				...
				<div class="rounded-lg border border-gray-200 bg-white p-6">
					...
					<p class="mt-1 text-sm text-gray-500">
						{formatCurrency(collectionStatus.pending.amount)}
					</p>
				</div>
				<div
					class="rounded-lg border border-gray-200 bg-white p-6"
				...
				>
					...
					<p class="mt-1 text-sm text-gray-500">
						{formatCurrency(collectionStatus.overdue.amount)}
					</p>
				</div>
			</div>
		{/if}
								...
								<tr class="border-b border-gray-100 hover:bg-gray-50">
									...
									<td class="py-3 font-medium">
										...
										{:else if property.electricity > 0 && property.water > 0}
											{property.electricity.toLocaleString()} kWh + {property.water.toLocaleString()}
											m³
										{:else}
											0
										{/if}
