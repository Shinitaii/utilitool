<script lang="ts">
	...
	const editData = $derived(
		crud.editFormData as unknown as {
			tenant_name: string;
					...
					{#each filteredData as item (item.id)}
						<tr class="border-b border-gray-200 hover:bg-gray-50">
							...
							<td class="px-6 py-4">
								<ActionButtons
									onEdit={() => {
										const startDate = toDate(item.tenant_start_date as any)
											.toISOString()
											.split('T')[0];
										const endDate = item.tenant_end_date
											? toDate(item.tenant_end_date as any)
													.toISOString()
													.split('T')[0]
											: undefined;
...
>
	<div class="space-y-4">
		<div>
			<label for="edit-tenant-name" class="block text-sm font-medium text-gray-700"
				>Tenant Name</label
			>
			<input
				id="edit-tenant-name"
