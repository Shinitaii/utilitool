export const billingPaths = {
  ...
  "/billings/{id}/purge": {
    delete: {
      tags: ["Billings"],
      summary: "Permanently delete an archived billing",
      description:
        "Second step of the archive-then-purge lifecycle (right-to-erasure). Only works on a " +
        "billing already soft-deleted via DELETE /:id. Admin-only.",
      security: [{BearerAuth: []}],
      parameters: [
        {
          name: "id",
      ...
      responses: {
        "204": {description: "Billing permanently deleted"},
        "401": {
          description: "Unauthorized",
        ...
        "409": {
          description: "Billing is still active — archive it first via DELETE /:id",
          content: {"application/json": {schema: {$ref: "#/components/schemas/ErrorResponse"}}},
        },
        "500": {
          description: "Internal server error",
